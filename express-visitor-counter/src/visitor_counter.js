...
module.exports = (config = {}) => {
  ...
  if (config.redisClient) {
    ...
    const redisSet = config.redisClient.flushdb
      ? (key) =>
          new Promise((resolve, reject) =>
            config.redisClient.set(key, 'OK', 'NX', 'EX', twoDays, (err, res) =>
    ...
    const syncWithRedis = (key, redisKey, action) =>
      redisKey
        ? redisSet(redisKey).then(
            (res) => res && action(key),
            (err) => {
              throw err;
            }
  ...
  return (req, res, next) => {
    ...
    const getPrefixedCounter = buildCounterId.bind(
      null,
      !config.withoutDate && todayDate,
      config.prefix || req.hostname
    );
    const getCounter = buildCounterId.bind(
      null,
      !config.withoutDate && todayDate
    );
    ...
    if (
      ...
      if (
        (!ipAddresses[ipAddressKey] ||
          !ipAddresses[ipAddressKey].processedToday) &&
        (!sessionIds[sessionKey] || !sessionIds[sessionKey].processedToday)
      ) {
        incCounter(
          getPrefixedCounter('visitors'),
          getCounter(req.ip, 'visitor')
        );
        !req.session.lastVisitDate &&
          incCounter(
            getPrefixedCounter('new-visitors'),
            getCounter(req.ip, 'new-visitor')
          );
      }
    ...
    if (!ipAddresses[ipAddressKey]) {
      ...
      incCounter(
        getPrefixedCounter('ip-addresses'),
        getCounter(req.ip, 'ip-address')
      );
    } else {
      ipAddresses[ipAddressKey].requests++;
      ipAddresses[ipAddressKey].processedToday =
        processedToday || ipAddresses[ipAddressKey].processedToday;
    }
    ...
    if (!sessionIds[sessionKey]) {
      ...
      incCounter(
        getPrefixedCounter('sessions'),
        getCounter(req.session.id, 'session')
      );
    } else {
      sessionIds[sessionKey].requests++;
      sessionIds[sessionKey].processedToday =
        processedToday || sessionIds[sessionKey].processedToday;
    }
    next();
  };
};
